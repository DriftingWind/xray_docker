...
if [ "$IPV4" != "null" ]; then
  SUB_IPV4="vless://$UUID@$IPV4:$EXTERNAL_PORT?encryption=none&security=reality&type=$NETWORK&sni=$FIRST_SERVERNAME&fp=firefox&pbk=$PUBLICKEY&flow=xtls-rprx-vision&path=$XHTTP_PATH&mode=auto#${IPV4}-xhttp"
  echo "IPV4 订阅连接: $SUB_IPV4" >> /config_info.txt
  echo -e "IPV4 订阅二维码:\n$(echo "$SUB_IPV4" | qrencode -o - -t UTF8)" >> /config_info.txt
fi
if [ "$IPV6" != "null" ]; then
  SUB_IPV6="vless://$UUID@$IPV6:$EXTERNAL_PORT?encryption=none&security=reality&type=$NETWORK&sni=$FIRST_SERVERNAME&fp=firefox&pbk=$PUBLICKEY&flow=xtls-rprx-vision&path=$XHTTP_PATH&mode=auto#${IPV6}-xhttp"
